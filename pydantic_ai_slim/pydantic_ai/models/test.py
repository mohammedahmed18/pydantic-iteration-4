...
class TestModel(Model):
    ...
    def _get_tool_calls(self, model_request_parameters: ModelRequestParameters) -> list[tuple[str, ToolDefinition]]:
        # Optimize to reduce intermediate object creation and minimize dictionary lookups
        call_tools = self.call_tools
        function_tools = model_request_parameters.function_tools
        if call_tools == 'all':
            # Fast path if calling all tools: just return [(name, tool), ...] directly
            return [(tool.name, tool) for tool in function_tools]
        else:
            # Precompute a lookup for name->tool
            function_tools_lookup = {tool.name: tool for tool in function_tools}
            # Avoid generator; use list-comprehension for both lookup and output tuple
