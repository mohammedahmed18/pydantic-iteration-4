...
class MistralStreamedResponse(StreamedResponse):
    ...
    def _validate_required_json_schema(json_dict: dict[str, Any], json_schema: dict[str, Any]) -> bool:
        ...
        # Short-circuit with fast path
        if not required_params:
            return True
        properties = json_schema.get('properties')
        if not properties:
            # If 'properties' missing, only check key presence
            return all(param in json_dict for param in required_params)

        # Local reference for type mapping
        ...
        for param in required_params:
            ...
            if not param_schema:
                continue  # If schema not defined, skip further checking
            ...
            if param_type == 'array':
                ...
                if not isinstance(value, list):
                    return False
                if param_items:
                    item_type = param_items.get('type')
                    if item_type:
                        elem_type = type_mapping[item_type]
                        # Avoid loop if list is empty
                        if value and any(not isinstance(item, elem_type) for item in value):
                            return False
            elif param_type:
                if not isinstance(value, type_mapping[param_type]):
                    return False

            # Only dive to nested if 'properties' exists in param_schema and value is dict
