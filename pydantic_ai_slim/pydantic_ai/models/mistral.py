...
class MistralStreamedResponse(StreamedResponse):
    ...
    def _validate_required_json_schema(json_dict: dict[str, Any], json_schema: dict[str, Any]) -> bool:
        ...
        valid_types = VALID_JSON_TYPE_MAPPING  # local reference for speed
        ...
        for param in required_params:
            ...
            if not param_schema:
                continue  # skip: no further validation if schema missing
            ...
            if param_type == 'array':
                ...
                if items_schema:
                    param_items_type = items_schema.get('type')
                    if param_items_type:
                        type_func = valid_types[param_items_type]
                        for item in val:
                            if not isinstance(item, type_func):
                                return False
            elif param_type:
                type_func = valid_types[param_type]
                if not isinstance(json_dict[param], type_func):
                    return False

            val = json_dict[param]
            # Fast path for possible nesting
            if isinstance(val, dict) and 'properties' in param_schema:
                nested_schema = param_schema
                if not MistralStreamedResponse._validate_required_json_schema(val, nested_schema):
                    return False
