...
class MistralStreamedResponse(StreamedResponse):
    ...
    def _try_get_output_tool_from_text(text: str, output_tools: dict[str, ToolDefinition]) -> ToolCallPart | None:
        # Short-circuit if pydantic_core.from_json fails (returns None or invalid type)
        output_json: dict[str, Any] | None = pydantic_core.from_json(text, allow_partial='trailing-strings')
        if not output_json:
            return None

        # Iterate directly over values; validation and return on first valid tool
        for output_tool in output_tools.values():
            if MistralStreamedResponse._validate_required_json_schema(
    ...
    def _validate_required_json_schema(json_dict: dict[str, Any], json_schema: dict[str, Any]) -> bool:
        ...
        for param in required_params:
            val = json_dict.get(param, None)
            if val is None:
                return False

            param_schema = properties.get(param)
            if not param_schema:
                # Property definition missing, cannot validate
                return False
            ...
            if param_type == 'array':
                ...
                # If "type" isn't specified for items, skip checking inner type
                items_type_name = item_schema.get('type') if item_schema else None
                if items_type_name:
                    checker = type_map.get(items_type_name)
                    if checker:
                        for item in val:
                            if not isinstance(item, checker):
                                return False
            elif param_type:
                checker = type_map.get(param_type)
                if checker and not isinstance(val, checker):
                    return False

            # Nested dict validation if applicable
